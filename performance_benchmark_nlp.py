...
class PerformanceBenchmark:
    """
    文本模型（直接接收原始文本）的高性能并发推理测试器
    从 Alpaca 格式 JSON 文件加载文本，通过 Triton HTTP 发送 BYTES 类型输入。
    """
    ...
    def _get_request_payload(self, text: str) -> dict:
        """将单个文本转换为 Triton v2 请求 payload（BYTES 类型）"""
        return {
            "inputs": [
                {
                    "name": "texts",
...
def parse_opt():
    parser = argparse.ArgumentParser(description="Trition 性能测试")
    parser.add_argument("--model", "-m", type=str, default="modernbert_ensemble", help="Triton ensemble模型名称")
    parser.add_argument("--backend", "-b", type=str, default="trt", choices=["onnx", "trt"], help="后端类型（仅用于文件名标识）")
    ...
    parser.add_argument("--max_batch_size", type=int, default=64, help="最大批次（仅用于文件名标识）")
    ...
    parser.add_argument("--num", "-n", type=int, default=1024, help="每次测试的总请求数")
    ...
    parser.add_argument("--max_seq_len" '-l', type=int, default=128, help="最大输入文本长度")
    parser.add_argument("--timeout", '-t', type=int, default=30, help="单次请求超时秒数")
    parser.add_argument("--verbose", "-v", action="store_true", help="打印详细错误信息及所选文本")
...
def main():
    ...
    output_csv = os.path.join(
        args.output_dir,
        f"{args.model}_{args.device}_{args.backend}_batch{args.max_batch_size}_cpu{args.cpu}_seqlen{args.max_seq_len}_benchmark_results.csv"
    )
    ...
    benchmark = PerformanceBenchmark(
        ...
        min_text_len=128,
        max_text_len=1024,      # 与原 load_text_from_alpaca 默认值一致
